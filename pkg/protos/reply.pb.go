...
func (x *Reply) GetId() string {
	if x, ok := x.GetReply().(*Reply_Id); ok {
		return x.Id
	}
	return ""
}

func (x *Reply) GetErr() string {
	if x, ok := x.GetReply().(*Reply_Err); ok {
		return x.Err
	}
	return ""
}
...
type Reply_Id struct {
	Id string `protobuf:"bytes,1,opt,name=id,proto3,oneof"`
}

type Reply_Err struct {
	Err string `protobuf:"bytes,2,opt,name=err,proto3,oneof"`
}

func (*Reply_Id) isReply_Reply() {}
